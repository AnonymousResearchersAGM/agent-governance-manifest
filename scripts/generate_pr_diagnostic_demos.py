...
def _artifacts(path,profile,*,include_test=True,conflict=False):
    ...
    elif path != "README.md":refs += ["O-RATIONALE","O-TEST-EXPLANATION"]
    ...
    if profile!="human_direct":result.append({"type":"agent_activity","title":"编码助手活动记录","content":json.dumps({"agent_type":"编码助手","task_summary":"修正 README 安装命令和失效链接" if path=="README.md" else "修复令牌撤销检查","files_modified":[path],"commands_executed":[] if path=="README.md" else ["python -m pytest tests/test_auth.py -q"],"tests_executed":[] if path=="README.md" else ["tests/test_auth.py"],"other_agents_or_tools_used":conflict,"network_access":False},ensure_ascii=False),"obligation_refs":["O-AGENT-SCOPE"]})
    if "auth" in path:
      if include_test:result.append({"type":"test_result","title":"测试记录","content":"python -m pytest tests/test_auth.py -q\n18 passed, 0 failed","source_tool":"pytest","obligation_refs":["O-TEST-COMMAND","O-ARTIFACT"]})
    ...
    if conflict:result.append({"type":"contribution_declaration","title":"贡献者声明","content":json.dumps({"used_other_agents_or_tools":False,"statement":"没有使用其他自动化工具。"},ensure_ascii=False),"obligation_refs":[]})
    return result
...
def _bridge(service,case,arts):
    ...
    SidecarEvidenceBridge(service).register_package(case.id,package_digest=receipt.package_digest);return receipt
...
def build_scenario(service,slug,path,profile,mode):
    ...
    if mode not in {"waiting","missing"} or (mode=="missing" and profile!="human_direct"):
      package=_bridge(service,case,_artifacts(path,profile,include_test=mode!="missing",conflict=mode in {"conflict","repair"}))
    if mode=="ready":
      ...
      loaded=service.storage.load_case(case.id); evidence=[item.id for item in loaded.evidence if item.evidence_type=="agent_action_scope"]
      activity=next(item for item in package.artifact_list if item.artifact_type=="agent_activity"); declaration=next(item for item in package.artifact_list if item.artifact_type=="contribution_declaration")
      service.record_sidecar_conflict(case.id,actor="maintainer-reviewer",role="maintainer",message="贡献者声明与编码助手活动记录不一致。",source_evidence_ids=evidence,source_artifact_ids=[declaration.artifact_id,activity.artifact_id],affected_obligation_ids=["O-AGENT-SCOPE"],package_digest=package.package_digest)
      ...
      _attest(service,case)
      service.request_repair(case.id,actor="maintainer-reviewer",role="maintainer",message="请更新测试记录。",affected_obligation_ids=["O-TEST-COMMAND"])
      service.resubmit(case.id,actor="contributor",role="contributor",summary="提交新版本。",affected_obligation_ids=["O-TEST-COMMAND"],diff_material="new deterministic contribution",change_classification="material")
    elif mode=="denied":
