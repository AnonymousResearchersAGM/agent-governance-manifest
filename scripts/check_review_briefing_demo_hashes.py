"""Check Review Briefing outputs against frozen cross-platform SHA-256 values."""
...
DEFAULT_OUTPUT = (
    REPOSITORY_ROOT / "examples" / "review_briefing" / "outputs"
)
DEFAULT_EXPECTED = (
    REPOSITORY_ROOT
    / "examples"
...
GENERATOR_VERSION = "review-briefing-phase-1-1-cross-platform"
COMMIT_BASIS = "0c22b54490a173e41ec5aef7f243e4d72faf3c85"
...
def expected_document(files: dict[str, str]) -> dict[str, Any]:
    return {
        "schema_version": "agm.review_brief_expected_sha256/v1",
...
def load_expected(path: Path) -> dict[str, str]:
    ...
    if (
        ...
        != "agm.review_brief_expected_sha256/v1"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Explicitly replace frozen values from existing outputs. "
            "Use only after reviewing intentional fixture changes."
        ),
    )
    ...
    try:
        ...
        return 2
    if problems:
        print("Review Briefing demo hash verification failed:")
        for problem in problems:
            print(f"- {problem}")
        return 1
    print(
        f"Verified {len(actual)} Review Briefing outputs against "
        f"{expected_path}."
    )
    return 0
